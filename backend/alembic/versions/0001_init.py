...
def upgrade() -> None:
    ...
    op.create_table("customers", sa.Column("id", sa.Integer, primary_key=True), sa.Column("name", sa.String(120), nullable=False), sa.Column("phone", sa.String(20)), sa.Column("address", sa.String(255)), sa.Column("status", sa.String(20), default="active"), sa.Column("pipeline_stage", sa.String(30), default="Lead"), sa.Column("assigned_user_id", sa.Integer, sa.ForeignKey("users.id")), sa.Column("notes", sa.Text), sa.Column("archived", sa.Boolean, default=False))
    ...
    op.create_table("expenses", sa.Column("id", sa.Integer, primary_key=True), sa.Column("expense_type", sa.String(50), nullable=False), sa.Column("amount", sa.Float, nullable=False), sa.Column("spent_at", sa.DateTime), sa.Column("note", sa.Text), sa.Column("deleted", sa.Boolean, default=False))
...
def downgrade() -> None:
    for t in ["update_logs", "audit_logs", "settings", "stock_ledger", "expenses", "payments", "sales_orders", "follow_ups", "customers", "roast_batches", "purchase_orders", "product_recipes", "products", "bean_types", "users"]:
        op.drop_table(t)
